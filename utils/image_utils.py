...
def load_pictures(pictures_dir: Path, mapping: Dict[str, str]) -> Dict[str, Optional[np.ndarray]]:
...
def draw_picture_with_border(frame: np.ndarray, picture: Optional[np.ndarray], phase_color: str) -> None:
    x0, y0 = 10, 10
    box_w, box_h = 160, 160

    if picture is None:
        cv2.rectangle(frame, (x0, y0), (x0+box_w, y0+box_h), (60,60,60), thickness=-1)
    else:
        ...
        roi = resized if resized.ndim == 3 and resized.shape[2] == 3 else cv2.cvtColor(resized, cv2.COLOR_BGRA2BGR)
...
def draw_label(frame: np.ndarray, label: str) -> None:
    cv2.putText(frame, label, (12, 190), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,255,255), 2, cv2.LINE_AA)
